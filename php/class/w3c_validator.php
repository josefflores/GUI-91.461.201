<?php
	...
	if( isset( $argv[2] ) ){
		...
		$root = array( 'http://www.josefflores.com/phpgui/' ) ;
		$current =  'http://www.josefflores.com/phpgui/validation/' ;
		$wv = new w3c_validator( $root , $current , '../lib/simple_html_dom.php' ) ;
		$wv->crawl() ;
		$wv->process() ;
		$response = $wv->report( ) ;
		
		$fp = fopen( 'validation.json' , 'w' ) ;
